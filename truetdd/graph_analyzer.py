...
#: A test that calls more than this many distinct source functions is flagged as a God Test.
GOD_TEST_THRESHOLD = 5
...
def analyze_traceability(graph_data: dict, source_dir: str = "src") -> dict:
    """
    ...
    Args:
        graph_data: Parsed graph.json dictionary.
        source_dir: Directory segment used to identify source (non-test) nodes.
                    Nodes whose ``source_file`` path contains this segment are
                    treated as source nodes. Test nodes are identified by the
                    presence of ``"test"`` or ``"tests"`` in the path parts.
    """
    ...
    for node_id, node in nodes_by_id.items():
        ...
        # Use Path.parts for exact segment matching — avoids false positives
        # like "contesting/" matching "test" or "srcutils/" matching "src"
        path_parts = set(Path(file_path).parts)
        is_test = bool(path_parts & {"test", "tests"})
        is_source = source_dir in path_parts
    ...
    test_to_source_calls: Dict[str, set] = {tid: set() for tid in test_nodes}
    source_to_test_callers: Dict[str, set] = {sid: set() for sid in source_nodes}
    ...
    for tid, callees in test_to_source_calls.items():
        if len(callees) > GOD_TEST_THRESHOLD:
    ...
    for tid, callees in test_to_source_calls.items():
        ...
        test_to_source_map[clean_label] = [
            nodes_by_id[c].get("norm_label", nodes_by_id[c].get("label", c)) for c in callees
        ]
...
def get_testdata_nodes(graph_data: dict) -> List[Dict]:
    """Return all nodes sourced from testdata.yaml files.

    These nodes represent named case sets declared in testdata.yaml sidecars.
    They are created by Graphify's semantic extractor when it processes
    YAML files named 'testdata.yaml'.

    Args:
        graph_data: parsed graph.json dict (from load_graph())

    Returns:
        List of node dicts whose source_file contains 'testdata'.
    """
    return [n for n in graph_data.get("nodes", []) if "testdata" in n.get("source_file", "")]


def get_testdata_coverage(graph_data: dict, test_node_id: str) -> Optional[str]:
    """Return the testdata set node ID linked to a test node, if any.

    Traverses graph links to find an edge from test_node_id to a node
    whose ID or source_file contains 'testdata'. Returns the target node ID
    or None if no testdata link exists.

    Args:
        graph_data: parsed graph.json dict (from load_graph())
        test_node_id: the graph node ID of the test function to query

    Returns:
        Target node ID (str) if a testdata link is found, else None.
    """
    for link in graph_data.get("links", []):
        if link.get("source") == test_node_id:
            target = link.get("target", "")
            if "testdata" in target:
                return str(target)
    return None
